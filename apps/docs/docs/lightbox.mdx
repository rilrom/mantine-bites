...
`Lightbox` is a full-screen image viewer built on top of `@mantine/carousel`. It uses a compound component pattern — slides are declared as `Lightbox.Slide` children. Each slide can have an optional `thumbnail` prop that renders in the thumbnail strip. Use `carouselOptions={{ initialSlide }}` to control which slide is displayed when the lightbox opens — click any image in the grid below to see it in action.
...
Embla options are passed through `carouselOptions.emblaOptions` and `thumbnailEmblaOptions`. These control carousel behavior such as `loop`, `dragFree`, `align` and more. See the [Embla Carousel options documentation](https://www.embla-carousel.com/api/options/) for the full list.
...
Customize the backdrop using `overlayProps`. Props are passed directly to Mantine's [`Overlay`](https://mantine.dev/core/overlay/) component. Defaults are `color: '#18181B'`, `backgroundOpacity: 0.9`, `zIndex: 200`:

<Demo data={demos.overlayProps} />

## Transition

Customize the open/close animation using `transitionProps`. Props are passed to Mantine's [`Transition`](https://mantine.dev/core/transition/) component. Defaults are `transition: 'fade'`, `duration: 250`:
...
- Zoom toggle includes `aria-label="Zoom in/out"`
